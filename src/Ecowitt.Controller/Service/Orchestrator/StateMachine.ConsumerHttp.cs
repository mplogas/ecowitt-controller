...
namespace Ecowitt.Controller.Service.Orchestrator
{
    public partial class StateMachine : IConsumer<HttpServiceEvent>
    {
        ...
        public Task OnHandle(GatewayApiData message)
        {
            ...
            var updatedGateway = message.Map(_sensorBuilder, _controllerOptions.Units == Units.Metric, _ecowittOptions.CalculateValues);
        ...
        public Task OnHandle(SubdeviceApiAggregate message)
        {
            ...
            foreach (var ip in ips)
            {
                ...
                {
                    var updatedSubDevice = data.Map(_sensorBuilder, _controllerOptions.Units == Units.Metric, _ecowittOptions.CalculateValues);
